 ...
 */
export const resolveProjectId = async (projectIdOrCode: any): Promise<mongoose.Types.ObjectId | null> => {
  if (!projectIdOrCode) return null;
  const str = typeof projectIdOrCode === 'string' ? projectIdOrCode.trim() : projectIdOrCode.toString();

  // 1. Dynamic lookup against Project model by projectId, projectCode, or name
  try {
    ...
    console.error('Error in resolveProjectId looking up code:', err);
  }

  // 2. Check if valid 24-character hex ObjectId
  if (mongoose.Types.ObjectId.isValid(str) && str.length === 24) {
    try {
      const projectById = await Project.findById(str);
      if (projectById) {
        return projectById._id as mongoose.Types.ObjectId;
      }
    } catch {
      // ignore
    }
...
/**
 * Resolves a project identifier to a list of query identifiers ([_id, codeString])
 * to reliably query parcels across schema variations.
 */
export const resolveProjectQueryIds = async (projectIdOrCode: any): Promise<any[]> => {
  if (!projectIdOrCode) return [];
  const str = typeof projectIdOrCode === 'string' ? projectIdOrCode.trim() : projectIdOrCode.toString();
  const resolved = await resolveProjectId(str);
  if (resolved) {
    return [resolved, str];
  }
  if (mongoose.Types.ObjectId.isValid(str) && str.length === 24) {
    return [new mongoose.Types.ObjectId(str), str];
  }
  return [str];
};

// Recalculates and synchronizes project.acquiredLand based on actual possessed parcels
export const syncProjectAcquiredLand = async (projectId: any) => {
  if (!projectId) return;
  try {
    const queryIds = await resolveProjectQueryIds(projectId);
    const list = await Parcel.find({ projectId: { $in: queryIds } });
    let totalAcquired = 0;
    for (const item of list) {
      const pos = (item.possessionStatus || '').trim().toLowerCase();
      const acq = (item.acquisitionStatus || '').trim().toLowerCase();
      const disb = (item.disbursementStatus || '').trim().toLowerCase();
      const comp = (item.compensationStatus || '').trim().toLowerCase();
      const isAcquired =
      ...
      if (isAcquired) {
        totalAcquired += Number(item.area) || 0;
      }
    }

    const resolvedId = await resolveProjectId(projectId);
    const targetId = resolvedId || projectId;
    if (mongoose.Types.ObjectId.isValid(targetId) && targetId.toString().length === 24) {
      await Project.findByIdAndUpdate(targetId, { acquiredLand: Number(totalAcquired.toFixed(2)) });
    } else {
      await Project.findOneAndUpdate(
        { $or: [{ projectId: targetId }, { projectCode: targetId }] },
        { acquiredLand: Number(totalAcquired.toFixed(2)) }
      );
    }
  } catch (err) {
    console.error('Error syncing project acquired land:', err);
  }
};
...
<<<<<<< HEAD
    ...
    if (parcelData.projectId) {
      ...
      if (resolvedId) {
        parcelData.projectId = resolvedId;
      }
    }
