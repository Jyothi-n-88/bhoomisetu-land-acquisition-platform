    ...
    <div className="bg-rose-50 rounded-xl border border-rose-200 p-6 shadow-sm">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-lg border border-rose-100 flex flex-col items-center text-center">
          <p className="text-3xl font-light text-rose-600 mb-1">{blockers.pendingCompensation ?? 0}</p>
          <p className="text-xs text-slate-600 font-medium leading-tight">Parcels Pending<br/>Compensation</p>
          {blockers.pendingCompensationAmount ? (
            <span className="text-[11px] font-semibold text-rose-700 mt-1.5 bg-rose-50 px-2 py-0.5 rounded border border-rose-200">
              ₹{(Number(blockers.pendingCompensationAmount) / 10000000).toFixed(2)} Cr
            </span>
          ) : null}
        </div>
        
        <div className="bg-white p-4 rounded-lg border border-rose-100 flex flex-col items-center text-center">
          <p className="text-3xl font-light text-rose-600 mb-1">{blockers.activeDisputes ?? 0}</p>
          <p className="text-xs text-slate-600 font-medium leading-tight">Active<br/>Legal Disputes</p>
        </div>
        
        <div className="bg-white p-4 rounded-lg border border-rose-100 flex flex-col items-center text-center">
          <p className="text-3xl font-light text-rose-600 mb-1">{blockers.pendingRnr ?? 0}</p>
          <p className="text-xs text-slate-600 font-medium leading-tight">Families Pending<br/>R&R</p>
        </div>

        <div className="bg-white p-4 rounded-lg border border-rose-100 flex flex-col items-center text-center">
          <p className="text-3xl font-light text-rose-600 mb-1">{blockers.pendingPossession ?? 0}</p>
