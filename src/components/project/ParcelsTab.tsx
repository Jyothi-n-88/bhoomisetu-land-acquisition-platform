          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-slate-50/50 text-xs uppercase tracking-wider text-slate-500 font-semibold border-b border-slate-200">
                ...
                <th className="px-6 py-4">Acquisition Status</th>
            ...
            <tbody className="text-sm divide-y divide-slate-100">
              {parcels.map((parcel) => (
                <tr key={parcel._id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4 font-mono">{parcel.area}</td>
                  <td className="px-6 py-4">
                    {getParcelStatusBadge(parcel.acquisitionStatus)}
                  </td>
                  <td className="px-6 py-4">
                    {parcel.disputeStatus === 'ACTIVE' ? (
                      <span className="flex items-center gap-1 text-xs font-medium text-rose-600 bg-rose-50 px-2 py-1 rounded-md border border-rose-200">
                        <AlertCircle className="w-3 h-3" /> ACTIVE
                      </span>
                    ) : (
                      <span className="text-xs font-medium text-slate-500">{parcel.disputeStatus}</span>
                    )}
                  </td>
                  {canEdit && (
