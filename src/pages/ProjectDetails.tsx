...
<<<<<<< HEAD
  ...
  const fetchProjectData = useCallback(async (isInitial = false) => {
    if (!id) return;
    try {
      if (isInitial) setLoading(true);
      
      const [projectRes, parcelsRes, blockersRes] = await Promise.all([
        getProject(id),
        getParcels(id),
      ...
      if (blockersRes.success) {
        setBlockers(blockersRes.blockers);
      }
    } catch (err: any) {
      if (isInitial) setError(err.message || 'Failed to load project details');
    } finally {
  ...
  useDataSync(useCallback((detail) => {
    // Refresh if event is global or matches current project
